...
(defun template-render (template-dirs template-name)
  "Render template NAME from TEMPLATE-DIRS."
  ...
  (let ((env (templatel-env-new
    ...
    (templatel-env-add-template
     en name
     (templatel-new-from-file (template-find template-dirs name)))
    (templatel-env-render env template-name vars)))

(template-render '("~/files/themes/templates/" "~/files/blog/templates/") "post.html")
    #+end_src
